...
export function parseDailyRecords(data) {
  ...
  const headers = data[0]; // ['kyle', 'CJ', '宝宣', '聪', 'yty', 'issac']
  const records = [];
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    headers.forEach((user, idx) => {
      ...
      if (typeof rank === 'number' && !isNaN(rank)) {
        dayRecord.rankings[user.toLowerCase().trim()] = {
          rank: rank,
...
export function parseCurrentWeek(data) {
  ...
  // 解析每日数据
  for (let i = 1; i <= 7 && i < data.length; i++) {
    const row = data[i];
    if (!row || row[0] === '总积分') {
      // 处理总积分行
      if (row && row[0] === '总积分') {
...
export function generateHeatmapData(dailyRecords) {
  ...
  return dailyRecords.map((day, idx) => {
    const bestRank = Math.min(...Object.values(day.rankings).map(r => r.rank));
    const worstRank = Math.max(...Object.values(day.rankings).map(r => r.rank));
    
    return {
      day: idx + 1,
      bestRank,
      worstRank,
      intensity: 7 - bestRank // 1-6 的反向，越早分数越高
    };
  });
}
